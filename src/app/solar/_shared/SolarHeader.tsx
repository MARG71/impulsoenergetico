...
export default function SolarHeader({
  logoSrc = "/brand/impulso-logo.png",
  links = [
  ...
  cta = { label: "Pedir estudio", href: "/solar/estudio" },
}: Props) {
  const [open, setOpen] = useState(false);

  const pill =
    "px-4 py-2 rounded-full bg-white/10 hover:bg-white/15 border border-white/10 transition text-base xl:text-lg font-extrabold";

  return (
    <header className="sticky top-0 z-50">
      {/* Fondo distinto para destacar */}
      <div className="bg-[#071B16]/90 backdrop-blur border-b border-white/10">
        <div className="mx-auto w-full max-w-[1600px] px-4 md:px-8 py-4 flex items-center justify-between gap-4">
          <Link href="/solar" className="flex items-center gap-3">
            <div className="relative h-11 w-11 rounded-2xl overflow-hidden bg-black/20 border border-white/10">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={cta.href}
              className="hidden sm:inline-flex items-center justify-center rounded-full bg-[#FFC107] hover:opacity-95 transition px-6 py-3 text-lg font-extrabold text-black"
            >
              {cta.label}
            </Link>
          ...
          <div className="lg:hidden border-t border-white/10">
            <div className="mx-auto w-full max-w-[1600px] px-4 md:px-8 py-4 grid gap-2">
              ...
              <Link
                href={cta.href}
                onClick={() => setOpen(false)}
                className="mt-2 px-4 py-4 rounded-2xl bg-[#FFC107] text-black text-lg font-extrabold text-center"
              >
                {cta.label}
              </Link>
            </div>
