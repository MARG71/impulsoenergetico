...
export default function ComparadorPage() {
  ...
  const [consumoPeriodos, setConsumoPeriodos] = useState({ P1: '', P2: '', P3: '', P4: '', P5: '', P6: '' });
  const [potencias, setPotencias] = useState({ P1: '', P2: '', P3: '', P4: '', P5: '', P6: '' });
  ...
  const ordenarResultados = (criterio: 'compañia' | 'ahorro' | 'comision') => {
    const resultadosOrdenados = [...resultados].sort((a, b) => {
      ...
      } else if (criterio === 'ahorro') {
        return b.ahorro - a.ahorro;
      } else if (criterio === 'comision') {
  ...
  useEffect(() => {
    const suma = periodosConsumo.reduce((acc, p) => acc + (parseFloat(consumoPeriodos[p] || '0')), 0);
    const anual = suma * 12;
    setConsumoAnual(anual.toFixed(2));
  }, [consumoPeriodos, nombreTarifa]);

  // 🔁 Cargar comparativa desde historial (por ID)
  useEffect(() => {
  ...
  // 🔁 Si no hay comparativaId pero hay idAgente e idLugar (desde QR), cargar nombres
  useEffect(() => {
  ...
  useEffect(() => {
    if (!comparativaId) {
      // Si no hay ID en la URL, limpia todos los campos para nueva comparativa
