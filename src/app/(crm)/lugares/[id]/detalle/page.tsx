...
export default function DetalleLugar() {
  ...
  if (!lugar)
    return (
      <p className="text-center text-red-600">Lugar no encontrado</p>
    );
  ...
  const comparativasFiltradas = (lugar.comparativas ?? []).filter(
    (c: any) => {
      const txt = `${c.id} ${c.tipoServicio} ${c.nombreTarifa} ${
        c.cliente?.nombre
      } ${c.agente?.nombre}`;
      return txt.toLowerCase().includes(busqueda.toLowerCase());
    }
    ...
    <div className="p-6 bg-[#E0F7FA] min-h-screen">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Image
            ...
            priority
          />
          <span className="hidden md:inline text-[#1F1F1F]">
            CRM · Detalle de lugar
          </span>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow mb-8 text-[#1F1F1F]">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <p>
            <strong>QR Code:</strong> {lugar.qrCode}
          </p>
          <p>
            <strong>Agente:</strong> {lugar.agente?.nombre || "-"}
          </p>
          <p>
            <strong>% Cliente:</strong> {fmtPct(lugar.pctCliente)}
          </p>
          <p>
            <strong>% Lugar:</strong> {fmtPct(lugar.pctLugar)}
          </p>
        </div>
        <div className="mt-4 flex gap-2">
          <Button
            onClick={() =>
              router.push(withTenant(`/lugares/cartel/${lugar.id}`))
            }
          ...
          {lugar.agenteId && (
            <Button
              onClick={() =>
                router.push(
                  withTenant(`/agentes/${lugar.agenteId}/detalle`)
                )
              }
