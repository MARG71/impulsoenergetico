...
export const runtime = 'nodejs' // para asegurar Node runtime en file upload

export async function POST(req: Request) {
  const formData = await req.formData()
  const tipoGlobal = (formData.get('tipo') as string || 'LUZ').toUpperCase()
  const subtipoGlobal = formData.get('subtipo') as string | null
  ...
  // Si el usuario marca "replace" y hay subtipo global, borramos todas las ofertas de ese subtipo previamente
  ...
  // Upsert ofertas base y borrado-recreación de tramos
  const upserts = await prisma.$transaction(
    Object.keys(group).map((key) => {
      const g = group[key]
      return prisma.ofertaTarifa.upsert({
        where: {
          // @@unique([tipo, subtipo, compania, nombre, anexoPrecio]) en Prisma
        // @ts-ignore - el nombre generado por Prisma coincide con los campos
